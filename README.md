...
### index.json is built and committed manually

`index.json` is **not** auto-generated by CI. It must be rebuilt locally and committed together with any extension changes. CI only validates that the committed `index.json` matches what the build script would produce — if they differ, the push is rejected.
...
Or add it as an npm script so you don't have to remember the path:
```json
{ "scripts": { "build": "node scripts/build-index.mjs" } }
```
```bash
npm run build
```

### First-party extension update workflow

```bash
# 1. Edit your extension files
# 2. Bump "version" in manifest.json (minor for features, major for breaking changes)
# 3. Rebuild index.json
node scripts/build-index.mjs
# 4. Commit everything together
git add addons/my-addon/ index.json
git commit -m "feat: my-addon — describe what changed"
git push
```

The build script reads the previous `index.json` and appends a new entry to `versions[]` if the version changed. Committing `index.json` is what makes the history permanent.

### Community extension update workflow

1. Edit your extension in your own repository.
2. Bump `"version"` in your `manifest.json`.
3. Rebuild your bundle: `npm run build`.
...
6. The maintainer runs `node scripts/build-index.mjs` locally and commits `index.json` when merging.
...
`index.json` is generated by `node scripts/build-index.mjs` and **committed manually** — always run the build script and include `index.json` in the same commit as your extension changes. CI validates that the committed file matches the build output and rejects pushes where they differ.
